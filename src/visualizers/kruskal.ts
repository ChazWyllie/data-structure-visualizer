  ...
  getCode(): CodeSnippets {
    return {
      typescript: [
        ...
        '',
        'function kruskal(nodeCount: number, edges: Edge[]): Edge[] {',
        ...
        '  const mst: Edge[] = [];',
        '  for (const edge of sortedEdges) {',
        '    if (union(edge.from, edge.to)) mst.push(edge);',
        '  }',
        '  return mst;',
        '}',
      ],
      python: [
        'def kruskal(node_count: int, edges: list[tuple[int, int, int]])',
      ...
      java: [
        'List<int[]> kruskal(int nodeCount, List<int[]> edges) {',
        ...
        '    for (int[] edge : edges) {',
        '        if (union(edge[0], edge[1])) mst.add(edge);',
        '    }',
        '    return mst;',
        '}',
      ],
